...
**Cenário C ou B (site novo):** persona pelo painel (`bun scripts/painel-api.ts POST /authors` e a foto), que é o **autor do WordPress** (`users.json` e `avatars/` da exportação), com o nome, a foto e a bio de lá, sem alegação de teste. É o autor que o Google associava ao texto quando ele rankeava, e o port muda o mínimo. Não troque para variar: no WordPress, cadeirasconfortaveis, melhorwifi e compraguia tinham o mesmo autor (Gustavo, com o mesmo Facebook no schema) e o mesmo tema (Astra), e apareceram juntos nas mesmas buscas, entre a 5ª e a 8ª posição, de 05/2025 a 04/2026 (medido em 26/09/2026). Dos 5 primeiros ports, só o melhorestetica trocou, de Ana Clara para Vanessa Correia, sem motivo registrado. `POST /sites/create` (modelo escolhido abaixo, `ratingStars` decidido na Fase 0) e a `site-criar-workflow` para o resto do scaffold. Se o `gitSync` voltar `commit-failed`, commitar na VPS com `PAINEL_AUTO_COMMIT=1` (aconteceu em 3 de 5 criações; o `detail` diz o motivo).
