...
1. `bun scripts/wp-portar.ts {export} sites/{site} --tag {tag} [--cadeia dominios-anteriores] [--irmaos dominios] [--so slugs] [--ano AAAA]` sem `--gravar`: todo post no formato, texto ≥99%, imagens 0 faltando, 0 link para outro site da rede. Depois com `--gravar`.
   ...
   - **"produto X sem ASIN"**: o card linka uma busca da Amazon (`/s?k=`), não um produto. Caso real: o ranking de marcas do compraguia (24/09/2026), com um card por marca. Conserte numa **cópia** do export: troque só o `href` de busca dentro do card pelo `/dp/` do modelo da foto do card (procure primeiro nos artigos do site; fora de linha, o sucessor, com o `sucessorDe` posto no `.mdx` depois do `--gravar`, porque o `wp-portar` não grava esse campo). Os links do texto ficam na busca. O `wp-portar` lê o texto do `posts.json` e grava o `port-plano.json` dentro do export: na cópia, esses dois (e a pasta `wp-content/uploads`, se você acrescentar foto) são próprios, nunca link para o original, senão a escrita cai no backup (aconteceu com o `port-plano.json`).
...
**Cenário C ou B (site novo):** persona pelo painel (`bun scripts/painel-api.ts POST /authors` e a foto), que é o **autor do WordPress**: quem assina os posts (campo `author` do `posts.json` da exportação; o outro usuário costuma ser o administrador, sem bio), com o nome, a foto (`avatars/`) e a bio de lá, sem alegação de teste. É o autor que o Google associava ao texto quando ele rankeava, e o port muda o mínimo. Não troque para variar: no WordPress, cadeirasconfortaveis, melhorwifi e compraguia tinham o mesmo autor (Gustavo, com o mesmo Facebook) e o mesmo tema (Astra), e apareceram juntos nas mesmas buscas de cadeira e de roteador de 05/2025 a 04/2026, na 1ª página (posição média de 5 a 13; de 5 a 8 no auge, de 10/2025 a 01/2026). Medido em 26/09/2026. Dos 5 primeiros ports, só o melhorestetica trocou, de Ana Clara para Vanessa Correia, sem motivo registrado. `POST /sites/create` (modelo escolhido abaixo, `ratingStars` decidido na Fase 0) e a `site-criar-workflow` para o resto do scaffold. Se o `gitSync` voltar `commit-failed`, commitar na VPS com `PAINEL_AUTO_COMMIT=1` (aconteceu em 3 de 5 criações; o `detail` diz o motivo).
- **Modelo: o que os vizinhos de busca não usam.** Vários sites da rede aparecem na mesma busca, e dois com a mesma cara lado a lado parecem o mesmo site (Marcelo, 26/09/2026). Rode `bun scripts/vizinhos-de-busca.ts {site}` depois de o scaffold existir e os artigos portados com keyword (ele lê as keywords dos `.mdx` e o histórico da cadeia em `grupos-legado.json`). O fim do relatório ordena t5, t6 e t7, os modelos que aceitam site sem página de produto, pelo que a família visual de cada um disputa. Escolha o primeiro. Se não for o criado, troque com `bun scripts/convert-site-template.ts {site} {tN}` (mostra o que muda) e `--apply`: ele troca os imports, o sites-meta e as opções do modelo no config. A troca à mão do melhortech (26/09/2026) esqueceu o `designVariant: 'v2'` do t5, e o site foi ao ar com o índice lateral em vez da barra no topo que todo t5 tem. Os 8 modelos são 3 na tela (A: t1 e t6; B: t2 a t5; C: t7 e t8): trocar t2 por t4 não muda a cara.
- Sem página de produto: `productPages: false` no config (tira o sitemap de produtos do índice) e o `[slug].astro` só com artigos, com a divergência em `docs/painel/_lib/template-divergences.ts`. O t6 esconde os links de produto por essa opção; o t5 e o t7 precisam também de `productPagesOnlyWithMdx: true`. Referência: melhoreseletro (t6) e melhortech (t5, trocado do t6 em 26/09/2026, com o mesmo `[slug].astro`).
- Trocar o modelo de um site portado só antes de ele rankear. Site que vai bem não troca de modelo (Marcelo, 26/09/2026): o modelo muda o HTML do site inteiro.
...
**Cenário A (herdeiro):**
- O herdeiro fica no modelo que já tem: ele rankeia, e trocar o modelo mexe no site inteiro.
...
- **O herdeiro muda de escopo com o port, e os textos do site não acompanham sozinhos.** No compraguia e no cozinhaideal (conferido em 25/09/2026, depois de os dois estarem no ar) sobraram:
  - no `config.ts`, o que o scaffold montou quando o site tinha 2 ou 3 artigos: H1 da home (`hero.titlePrefix + titleHighlight + titleSuffix`, que saiu "O melhor compra para você" e "O melhor cozinha para você"), `hero.subtitle` ("Comparamos Cozinha das principais marcas."), `description` (meta da home e `llms.txt`) e `contactMainTopic`. O `contactMainTopic` foi gravado DENTRO do `src/content/pages/contato.html` na criação ("Dúvidas sobre qual cozinha comprar"): trocar só o config não muda a página, edite a frase no arquivo também;
  - o `/sobre/` e o `/author/` com o escopo de antes (o do cozinhaideal falava em "eletroportáteis" com fogão, geladeira e lava-louças no site). Refazer as DUAS páginas pela `preencher-institucionais`: no herdeiro o /sobre/ nunca foi do WordPress, e a exceção de "site portado" dela não vale.
...
1. `bun scripts/port-regras.ts {dominio-wp} {site} --incluir-hostinger --extra={arquivo.tsv}` — mostra as regras da cadeia inteira, os endereços que vão para a home e as regras do site que escondem artigo. O que tiver destino óbvio vai para o `--extra` (erro de digitação, home que era o próprio artigo no Arquivo da Internet, categoria renomeada). **Nunca a página inicial para slug de artigo:** sem regra própria, a regra geral leva o endereço ao mesmo caminho no site novo, que chega à home em 2 saltos enquanto não há artigo, e ao artigo quando ele for criado (recuperação de órfãs). Regra para a home vale antes da regra geral e segura o slug. Regra para a home só em listagem do WordPress (`page/2`, `category/…`, `melhores`). Depois `--gravar` (e `--tirar-escondidas` no cenário A) e commit. **Não publique o worker ainda.**
   ...
   - **Herdeiro cuja cadeia já foi virada antes** (compraguia, 24/09/2026: "o domínio continua o mesmo, só adicionar os artigos"): o port-regras sai com 0 regra nova e o `--gravar` recusa por causa dos conflitos, que aqui são sugestões de destino melhor para regras que já existem (categoria da cadeia que ia para a home e agora existe no site). Mudar essas regras não é o pedido. Tire só as regras que escondem os portados direto no `worker/redirects.json`, filtrando pelo host do site: a mesma slug tem regra em outros domínios da rede (no compraguia, as 9 slugs tinham regra também em 5 outros: escritorioecasa.com, escritoriocasa.com.br, impressoraideal.com, melhoraspirador.com e melhorguia.com.br). Confira que o arquivo resultante é a lista original menos elas, e rode o port-regras de novo sem `--gravar`: não pode sobrar nenhuma escondida. Os conflitos e os endereços com destino óbvio vão para o relatório, para decisão do Marcelo.
...
16. **Modelo fixo.** A skill mandava t6 para todo site criado pelo port, porque em 19/09 era o único modelo sem página de produto. Os 5 primeiros saíram todos em t6, e melhoreseletro e melhortech disputavam 4 buscas de celular com a mesma cara. O melhortech foi para o t5 em 26/09/2026, antes de rankear. Hoje o modelo sai do `vizinhos-de-busca.ts` (Fase 3).
