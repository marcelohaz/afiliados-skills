...
1. `bun scripts/wp-portar.ts {export} sites/{site} --tag {tag} [--cadeia dominios-anteriores] [--irmaos dominios] [--so slugs] [--ano AAAA]` sem `--gravar`: todo post no formato, texto ≥99%, imagens 0 faltando, 0 link para outro site da rede. Depois com `--gravar`.
   ...
   - **"produto X sem ASIN"**: o card linka uma busca da Amazon (`/s?k=`), não um produto. Caso real: o ranking de marcas do compraguia (24/09/2026), com um card por marca. Conserte numa **cópia** do export: troque só o `href` de busca dentro do card pelo `/dp/` do modelo da foto do card (procure primeiro nos artigos do site; fora de linha, o sucessor com `sucessorDe`). Os links do texto ficam na busca. O `wp-portar` lê o texto do `posts.json` e grava o `port-plano.json` dentro do export: na cópia, esses dois (e a pasta `wp-content/uploads`, se você acrescentar foto) são próprios, nunca link para o original, senão a escrita cai no backup (aconteceu com o `port-plano.json`).
