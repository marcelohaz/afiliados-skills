...
description: Escreve a descrição de VÁRIAS categorias de uma vez, agrupando por categorySlug — não por site. Qualidade idêntica à `categoria-descricao-escrever`, que é a régua viva (a skill mãe NÃO reimplementa nada dela). Só escreve em categoria com ≥1 artigo FINALIZADO: o texto é ancorado nos artigos (§1 cita os comparativos, §2 lista os perfis que eles cobrem), então sem artigo ele prometeria o que não existe — as demais entram no relatório como fila de artigos a escrever. Paralelismo é POR GRUPO de categoria (glutamina em 7 sites, termogênico em 6): grupos correm em paralelo, DENTRO do grupo é sequencial e cada item recebe o texto dos anteriores, senão saem N descrições irmãs = duplicate content. Sub-agents devolvem HTML como DADO e não tocam em arquivo (todas as descrições de um site moram no mesmo config.ts — escrita paralela se sobrescreveria); a mãe encadeia writeCategoryDescription em memória e grava 1x por site, 1 commit. Aceita `--site X` pra limitar. Pré-flight via `bun scripts/categoria-desc-alvos.ts`.
---
