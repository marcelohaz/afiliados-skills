...
6.5. **Comparar duplicata cross-site** (mede, não adivinha): a criação escreve a página livremente pela bíblia, sem tentar "ser diferente" de outros sites. **É AQUI que a comparação acontece.** Rode o comparador (auto-descobre páginas do MESMO ASIN em outros sites nossos):
   ...
   **Antes de ler o resto: `target_ilegivel: true` (exit 2) = a página não pôde ser parseada → 🔴 YAML inválido (critério 6) e comparação PENDENTE; `peers_ilegiveis > 0` = irmão pendente. Nenhum dos dois é "limpo" (corrigido 2026-09-01; ver critério 19).**

6.7. **Camada MECÂNICA determinística PRIMEIRO** (canon 2026-07-24 — não conta char de cabeça): rode o script antes de julgar. Ele decide 100% dos checks contáveis/estruturais (tamanho texto-puro, fence, travessão, `;` em prosa entity-aware, HTML em campo texto-puro, 4 rótulos do fullReview, termos banidos absolutos). Motivo: LLM erra ~1/3 desses (medido no gabarito: 6 de 19 shortDescriptions >250 vivas passaram batido na auditoria LLM; o script pega 100%). Desde 2026-09-01 ele também emite `voltagem-citada` e `bivolt-sem-confirmacao` (lê o `specsAmazon` da bíblia do ASIN); são a entrada do critério 21, e `termo-banido` não dispara mais quando o termo faz parte do `name`.
   ```bash
