...
- **subtitle = KEYWORD-FIRST (v1.52.0), NÃO ângulo.** O prompt do sub-agent DEVE pedir: keyword INTEIRA do artigo + cauda variada ("{keyword} custo-benefício", "{keyword} profissional", "{keyword} da {Marca}", "{keyword} boa e barata"), 5-7 palavras, SEM spec técnica (sem Hz/GB/mAh/polegada). NÃO instruir "o ângulo do produto pra X" (erro real 2026-06-24: pedir ângulo gerou subtitle estilo "Topo do Android com AMOLED 120Hz"). Ver régua canônica em artigo-review-criar → "subtitle — rótulo SEO KEYWORD-FIRST".
