...
4. **1.3 Anti-dup intra-site** (auto): cada review tem que divergir de **DUAS frentes**, não uma (jaccard de sentenças, limite 0.80):
   - **(a) a página individual** do mesmo produto no destino.
   - **(b) o review do MESMO ASIN em qualquer OUTRO artigo do mesmo site** (`sites/{target}/src/content/reviews/*.mdx`). ⚠️ **Esta é a frente que costuma faltar e ela cresce a cada artigo da fila.** Num clone em fila, o artigo nº 5 já encontra a maior parte dos produtos com 1-3 reviews prontos no próprio site. Medido no compraguia 2026-07-30, na metade da fila: **34 de 49 reviews restantes (69%)** tinham irmão intra-site. O gate 1.4 achou tone-clone real em 4 de 5 produtos compartilhados de um item, e num deles o overlap era de **35 6-gramas**.
   - **Passe pro sub-agent a LISTA de ângulos já ocupados** por aquele ASIN (um resumo de uma linha por texto existente), não só o caminho dos arquivos. Sem isso ele escolhe um ângulo às cegas e colide.
   - Acima do limite → auto-fix (reescreve trecho divergindo) → re-valida.
   - ⚠️ **TETO — não force contorção.** Quando um produto já acumula 3-4 reviews no mesmo site, o espaço de ângulos honestos acaba. A régua aqui é a mesma que vale cross-site (`afiliados.regras.pagina-produto-sobreposicao-crosssite-ok`): **medir, reportar e divergir onde há material**, aceitando sobreposição residual onde não há. Torcer o texto pra fugir de um irmão é pior que a sobreposição — spec é spec, e número factual repete mesmo. Se não houver ângulo novo legítimo, registre no relatório em vez de inventar.
...
2. **5.2** Trechos que sobraram acima do limite (exatas > 0 OU jaccard ≥0.8) → corrija, mas **escolha o meio pelo tamanho do trecho**:
