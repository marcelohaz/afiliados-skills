...
0a. **Decide a SLUG do destino AGORA, antes do `init` (canon Marcelo 2026-09-12).** O clone herda a slug do FONTE; o histórico no Google é do DESTINO. São coisas diferentes, e a diferença custa tráfego. Dois casos medidos: o `amelhorimpressora` (28/08) recebeu as duas maiores páginas do domínio em slugs de 0 e 6 impressões; o `compraguia/melhor-climatizador` (12/09) nasceu numa slug sem histórico enquanto `/melhor-climatizador-de-ar/` do próprio domínio acumulava 28.725 impressões e 186 cliques em 16 meses (461 nos últimos 28 dias).
   ...
   Leia a keyword do fonte (`grep -m1 '^keyword:' sites/{source}/src/content/reviews/{slug-fonte}.mdx`) e o mapa de órfãs (`docs/painel/_data/mapa-artigos-orfaos.json` → `sites[{target}]`, campos `slug`/`imp16m`/`cli16m`/`imp28d`). O `slugify` é o de `scripts/audit-slugs.ts:102` (sem acento, minúsculo, todo não-alfanumérico vira hífen). **Renomeia só quando as CINCO condições valem juntas:**

   ```
   alvo = slugify(keyword)      plan = slug que veio do fonte

   alvo == slug de uma órfã do PRÓPRIO destino                    ← IGUALDADE, nunca semelhança
   alvo != plan
   órfã(alvo).imp16m >= 1000  E  >= 3 × órfã(plan).imp16m         ← PISO_IMP e FATOR do audit-slugs.ts (plan fora do mapa = 0)
   alvo não coincide com categorySlug do destino, nem no singular/plural  ← é listagem antiga, não artigo
   alvo livre em sites/{target}/src/content/reviews/
   ```
   ...
   **A igualdade é a guarda que mais trabalha, e o contraexemplo é o air fryer.** O `cozinhaideal` tem o artigo `melhor-air-fryer` e duas órfãs muito maiores do que ele: `melhor-air-fryer-12-litros` (96.235 imp) e `melhor-air-fryer-custo-beneficio` (56.230). A tentação é pôr o artigo na órfã de 96 mil. São **keywords diferentes** (guarda 3 do `audit-slugs.ts`): sequestrar a slug da irmã joga o artigo genérico numa URL cuja busca é outra E queima a slug do artigo que deveria nascer ali. A guarda `orfaEhFormaCurtaDe` (`audit-slugs.ts:414`) **não substitui** a igualdade — medido em 12/09, ela sozinha aprovaria 3 movimentos que a igualdade barra, entre eles mandar o `melhor-aspirador-de-po-vertical` pra órfã `melhor-aspirador-de-po` (89.159 imp), que é o artigo genérico, não o vertical.
   ...
   Medição da régua em 12/09 sobre os 404 artigos da rede: **0 disparos indevidos**, concorda com as 3 slugs históricas já adotadas (`impressora-para-fotos` 214.870, `melhor-impressora-tanque-de-tinta` 111.696, `melhor-climatizador-de-ar` 28.725), e o piso barra 3 candidatas de 99, 77 e 2 impressões em 16 meses. **Controle positivo:** simulando todo par fonte→destino da rede como se os artigos ainda não existissem, a régua reconstrói exatamente os 3 incidentes reais, inclusive as slugs erradas originais do `amelhorimpressora` (`impressora-de-fotos`, `melhor-impressora-tanque-tinta`). O fator e a categoria não mudam nenhum dos 3 e não têm exposição hoje; estão aqui porque o padrão existe no mapa e custam uma comparação cada.

   **Só vale pra artigo que ainda não existe.** Artigo que já está no ar nunca muda de slug: a URL histórica ganha 301 para ele (canon 2026-09-12, ver o passo de slug da `linkagem-auditar`).

   **Por que aqui e não no fim:** a slug entra no nome do clone-log (0b), dos 2 marcadores de auditoria e do `.mdx`, e o **gate de invocação** do `verify-output` procura o slug no transcript. Renomear com o artigo pronto custou 4 `git mv` e invalidou o gate das 5 skills (caso real de 12/09, registrado como desvio no clone-log daquela run). Aqui custa uma variável. **Com `RETOMAR=yes`, NÃO recalcule** — a condição "livre" passa a falhar contra o próprio `.mdx` em construção e a régua devolveria a slug do fonte. Os args do heartbeat também trazem a slug do FONTE. **Ache a slug da run pelo log:** se `docs/biblias-v2/.audits/clone-runs/{target}-{alvo}-last.md` existe com a linha `- **Fonte:** {source}/{plan}` (é assim que o `init` grava), a slug é `alvo`; senão, é `plan`. Use essa slug em todo `{slug}` da retomada (git log, `verify`, reviews em `rev/`). Com `FILA=yes`, a fila já resolveu a slug por esta mesma régua antes de abrir o log — **confira que bate** e siga; se divergir, a régua é a daqui e a fila é quem se realinha (a decisão é determinística: mesma keyword + mesmo mapa de órfãs = mesma slug).
...
- **Slug (linha OBRIGATÓRIA, saia ela como sair — mudou ou não).** É o que deixa um rename errado visível na hora, pro Marcelo corrigir no mesmo turno em vez de descobrir semanas depois no GSC:
  ...
  - não mudou → `🔗 **Slug:** \`melhor-x\` — mantida (nenhuma órfã do destino bate com slugify(keyword) acima do piso de 1.000, ou a slug do fonte já tem o histórico maior)`
