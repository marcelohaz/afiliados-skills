...
- **git-verdade antes de cada item (idempotência):** `git log --oneline -- sites/{target}/src/content/reviews/{slug}.mdx`. Se já tem commit → **PULA** (marca "já feito" no relatório). Se existe `.mdx` não-commitado → regenera (trabalho interrompido). Re-rodar a fila é seguro.
  - ⚠ **Confira TAMBÉM pela keyword, não só pela slug** (desde a Etapa 0a da clone, 12/09): `grep -l '^keyword: "{keyword do fonte}"' sites/{target}/src/content/reviews/*.mdx`. Achou qualquer arquivo → o artigo **já existe** naquele destino, com outra slug, e o item é `PULAR`. Sem isso, um item cuja slug foi adequada ao histórico volta a `FAZER` numa re-rodada e a fila cria o artigo DUAS vezes — a identidade do artigo é a keyword, a slug é só o nome do arquivo.
