...
0.5. **Carregar chavões do nicho** (régua v1.18.0, expandida v1.19.0):
   ...
   - **`voltagem_bivolt` (régua dura, canon 2026-06-28)** → NUNCA afirme "bivolt"/"funciona em qualquer tomada"/"sem transformador" se o `specsAmazon`/`descricaoProduto` do ASIN traz voltagem ÚNICA (ex: "Voltagem 110 Volts") e não diz "bivolt" explícito. Copy de potência tipo "1800W 110V | 2000W 220V" ou "110/127V e 220V" = SKUs SEPARADOS, não bivolt. **Aparelho de aquecimento de alta potência (air fryer, ferro, secador, chaleira) é voltagem única por design.** Na dúvida, **OMITA a voltagem** (não cite na spec nem na prosa) — melhor omitir que afirmar bivolt errado. Se há versões 110V e 220V, no máximo "vendido em versões 110V e 220V, confira no anúncio". Exceção (bivolt legítimo): impressora e cooktop a gás. Caso real: air fryers NA341/Midea/Mondial/WAP afirmados bivolt erradamente (2026-06-28).
