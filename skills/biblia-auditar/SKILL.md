...
4.5. **Carimbar a auditoria + gravar `auditFlags` na bíblia (SEMPRE, mesmo read-only)**: backup (`cp docs/biblias-v2/<ASIN>.json docs/painel/.painel-backups/$(date +%Y-%m-%d)/<ASIN>-v2-$(date +%H%M%S).json`), depois script que lê o JSON e seta, no mesmo write:
   ...
   - **`b.auditFlags`** = `[{ type, label }]` com os achados SEMÂNTICOS **report-only** que sobram — acendem o chip na coluna Observações do painel (o detector mecânico `contaminado` só pega marca/ASIN; estes são os que só a auditoria vê). `type`: **`'wrong-info'`** (erro que compromete a IDENTIDADE do produto: (a) a captura da Amazon é de OUTRO produto — `specsAmazon`/ficha técnica descrevem outro aparelho, ou o ASIN na ficha técnica diverge do ASIN da bíblia — OU (b) um FATO ERRADO afirmado num campo CURADO sobre ESTE produto. ⚠️ **NÃO é `wrong-info`**: mera divergência de atributo entre a captura da Amazon e o fabricante (CPU/tela/RAM/SO/bateria/peso capturado errado), erro de captura/listagem da Amazon, ou material de variante-irmã no `conteudoBrutoFabricante` — quando o ASIN/produto da captura está CORRETO isso é `'review'`, **mais ainda quando já existe `dadosInconsistentes.decisaoEditorial` resolvendo** o caso. Critério-âncora: bloqueia (`wrong-info`) só se o ASIN/produto capturado está errado; atributo divergente com ASIN certo não bloqueia — o review já é instruído pela `decisaoEditorial`), **`'off-niche'`** (tipo de produto do bruto contradiz a `categoria`/`subcategoria` da PRÓPRIA bíblia — raro; NÃO é "produto no site errado"), **`'review'`** (frescor / verificação externa / sem valor certo / **divergência de campo-Amazon: `specsAmazon`×fabricante, erro de captura/listagem da Amazon, ou variante-irmã no bruto, com ASIN/produto correto — especialmente a já coberta por `dadosInconsistentes`**). `label` ≤ ~120 ch com o motivo concreto, sem aspas duplas. **Só achado report-only vira flag** (o que vai ser auto-fixado na Etapa 9 NÃO entra). **Se nada qualifica → `b.auditFlags = []`** (OBRIGATÓRIO esvaziar — re-auditar depois do conserto APAGA o chip; chip preso = bug).
   - Write `JSON.stringify(b, null, 2) + '\n'`.
   Vale mesmo read-only (sem fix). **Contaminação cross-produto** = dado de produto de OUTRA marca/linha vazado na captura (a ficha técnica é de outro aparelho) = flag `'wrong-info'` que persiste até re-captura. Mas **atributo capturado errado do PRÓPRIO produto (ASIN certo) = `'review'`, não `'wrong-info'`** — não bloqueia, ainda mais com `dadosInconsistentes` resolvendo. (Se DEPOIS aplicar fixes na Etapa 9, re-bumpa o lastModified lá; tudo bem.)
