...
4.5. **Carimbar a auditoria + gravar `auditFlags` na bíblia (SEMPRE, mesmo read-only)**: backup (`cp docs/biblias-v2/<ASIN>.json docs/painel/.painel-backups/$(date +%Y-%m-%d)/<ASIN>-v2-$(date +%H%M%S).json`), depois script que lê o JSON e seta, no mesmo write:
   ...
   - **`b.auditFlags`** = `[{ type, label }]` com os achados SEMÂNTICOS **report-only** que sobram — acendem o chip na coluna Observações do painel (o detector mecânico `contaminado` só pega marca/ASIN; estes são os que só a auditoria vê). `type`: **`'wrong-info'`** (dado de OUTRO produto em qualquer campo, inclusive bruto / contradição factual / claim que contradiz o bruto), **`'off-niche'`** (tipo de produto do bruto contradiz a `categoria`/`subcategoria` da PRÓPRIA bíblia — raro; NÃO é "produto no site errado"), **`'review'`** (frescor / verificação externa / sem valor certo). `label` ≤ ~120 ch com o motivo concreto, sem aspas duplas. **Só achado report-only vira flag** (o que vai ser auto-fixado na Etapa 9 NÃO entra). **Se nada qualifica → `b.auditFlags = []`** (OBRIGATÓRIO esvaziar — re-auditar depois do conserto APAGA o chip; chip preso = bug).
   - Write `JSON.stringify(b, null, 2) + '\n'`.
   Vale mesmo read-only (sem fix). Contaminação cross-produto em campo bruto = flag `'wrong-info'` que persiste até re-captura. (Se DEPOIS aplicar fixes na Etapa 9, re-bumpa o lastModified lá; tudo bem.)
