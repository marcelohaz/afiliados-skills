...
**Flag opcional `ANINHADA=yes`**: passado quando esta skill roda DENTRO de outra em-massa (ex.: a `artigo-clonar-em-massa` criando páginas faltantes no pré-flight 5). Efeito único: **NÃO armar o heartbeat próprio** — há UM só despertar pendente por sessão e o seu substituiria o da mãe, matando a retomada dela (mesma mecânica do `FILA=yes` da clone). Sem a flag mas com outra em-massa comprovadamente viva na sessão, vale a mesma regra — foi o que 2 execuções fizeram por conta própria em 17 e 19/08, e estava certo; agora é régua, não improviso.
