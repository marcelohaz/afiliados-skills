...
- **SEM aviso de saúde na intro** (régua v1.34.0). Mesmo em nicho de suplemento/saúde, a intro NÃO leva "consulte um médico/nutricionista" nem disclaimer YMYL — isso fica 1× no guia (ver `artigo-guia-escrever`). Repetir o aviso na intro + guia + reviews é a ladainha que a `artigo-auditar` (`disclaimer-saude-repetido`) flagra.
