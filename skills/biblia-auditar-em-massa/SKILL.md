...
descreve conflito EXTINTO (o registro diz "o campo X informa V" e V não está mais em X, porque o bruto foi
re-capturado depois da curadoria) → **consolidar num registro único que preserve o histórico e as decisões
editoriais que continuam valendo**, nunca apagar seco. Direção conhecida: o bruto atual manda. Item novo do
`regras-biblia.md` §4, que segue sendo a régua desta skill.
...
- ⛔ **Prompt do re-auditor: "NÃO grave nada" + a mãe CONFERE.** Em 27/08 um re-auditor gravou na bíblia apesar da proibição (B0CG2HZ7B1). A instrução não basta: após cada ciclo, a mãe compara o mtime (ou hash) do JSON com o de antes do ciclo — mudou, restaura do backup e trata como reprova.
- **O re-auditor confere contra o ARQUIVO, nunca contra a declaração da mãe.** Em B09WYYM8Y4 a mãe declarou aplicado um conserto que não aplicou, e o ciclo 3 só pegou porque leu o arquivo. Com 4+ consertos por bíblia, a mãe confere a aplicação um a um antes de declarar.
...
- **`b.auditFlags`** = avisos SEMÂNTICOS que SOBRARAM pós-conserto, gravados na bíblia (`wrong-info`/`off-niche` acendem o chip na coluna Observações; `review` fica no relatório/editor) (o detector mecânico `contaminado` só pega marca/ASIN; estes são os achados que só a auditoria vê). Cada flag `{ type, label }` (label ≤ ~120 ch, sem aspas duplas, com o motivo concreto):
