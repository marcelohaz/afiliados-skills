...
- **Marque cada (C) com `flagType`** (vira o chip do painel na Etapa 3.6): `'wrong-info'` (erro que compromete a IDENTIDADE do produto: (a) a captura da Amazon é de OUTRO produto — `specsAmazon`/ficha técnica de outro aparelho, ou o ASIN na ficha diverge do ASIN da bíblia — OU (b) FATO ERRADO num campo CURADO sobre ESTE produto. ⚠️ **NÃO é `wrong-info`**: divergência de atributo entre a captura da Amazon e o fabricante (CPU/tela/RAM/SO/bateria/peso capturado errado), erro de captura/listagem da Amazon, ou material de variante-irmã no `conteudoBrutoFabricante` quando o ASIN/produto está CORRETO → isso é `'review'`, mais ainda com `dadosInconsistentes.decisaoEditorial` resolvendo. Âncora: bloqueia só se o ASIN/produto capturado está errado), `'off-niche'` (tipo de produto do bruto contradiz a `categoria`/`subcategoria` declarada NA PRÓPRIA bíblia — raro; NÃO é "produto no site errado", isso a bíblia não sabe), `'review'` (frescor / verificação externa / sem valor certo / **divergência de campo-Amazon com ASIN correto — especialmente a já coberta por `dadosInconsistentes`**). Contaminação cross-produto (ficha de OUTRO aparelho/marca) = `'wrong-info'`; atributo errado do próprio produto (ASIN certo) = `'review'`.
...
⚠ `auditFlags` é o que faz o chip ⚠/🧭/🔍 aparecer no painel (server lê `d.auditFlags`; `_pages/biblias.ts` renderiza). NUNCA inventar flag sem evidência. **Contaminação cross-produto** (ficha de OUTRO aparelho/marca vazada na captura) fica como flag `'wrong-info'` (não dá pra consertar curando — só re-captura limpa) até alguém recapturar. Mas **atributo capturado errado do PRÓPRIO produto (ASIN certo) = `'review'`, não `'wrong-info'`** — não bloqueia, ainda mais com `dadosInconsistentes` resolvendo.
